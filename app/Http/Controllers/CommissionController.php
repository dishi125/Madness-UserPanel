...
class CommissionController extends Controller
{
    public function index(){
        return view('monthly_commissions.list');
    }
    ...
    public function allMonthlyCommissionlist(Request $request){
        if ($request->ajax()) {
            ...
            if (isset($commission_status)){
                $totalData = MonthlyCommission::where('user_id',Auth::user()->id)->whereIn('commission_status',$commission_status)->count();
            }
            ...
            if(!empty($MonthlyCommissions))
            {
                foreach ($MonthlyCommissions as $MonthlyCommission)
                {
                    ...
                    $nestedData['action'] = '<button class="btn gradient-9 btn-sm ViewMonthlyCommissionBtn" data-id="'.$MonthlyCommission->id.'"><i class="fa fa-eye" aria-hidden="true"></i></button>';
                    $data[] = $nestedData;
                }
            }
    ...
    public function viewMonthlyCommission($id){
        return view('monthly_commissions.view',compact('id'));
    }
