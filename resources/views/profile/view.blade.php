...
{{--                        <br>--}}
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            ...
                            <div class="row">
                                <div class="col-sm-12 text-right">
                                <button type="button" id="ChangePwdBtn" class="btn btn-outline-dark" data-toggle="modal" data-target="#ChangePwdModal" data-id="{{ $user->id }}">Change Password</button>
                                </div>
                            </div>

                            <div class="media align-items-center mb-4">
                                @if(isset($user->profile_pic))
                                <img class="mr-3" src="{{ env('DISPLAY_IMAGE_URL').'profile_pic/'.$user->profile_pic }}" width="80" height="80" alt="Profile" id="Profile_pic_val">
                                @else
                                <img class="mr-3" src="{{ url('public/images/default_avatar.jpg') }}" width="80" height="80" alt="Profile">
                                @endif
                                <div class="media-body">
                                    <h3 class="mb-0" id="full_name">{{isset($user->first_name)?$user->first_name:''}} {{$user->last_name?$user->last_name:''}}</h3>
                                </div>
                            </div>
                            ...
                            <div class="row custom-row">
                                <div class="col-sm-4">
                                    <b>Membership<span class="editorderListGem"></span></b>
                                </div>
                                <div class="col-sm-8"><span class="label label-warning">{{ ($user->is_premium==0)?'Free':'Premium' }}</span></div>
                            </div>
                            {{--<div class="row custom-row">
                                <div class="col-sm-4">
    ...
    <div class="modal fade" id="ChangePwdModal">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form class="form-valide" action="" id="passwordform" method="post">
                    {{ csrf_field() }}
                    <div class="modal-header">
                        <h5 class="modal-title" id="formtitle">Change Password</h5>
                        <button type="button" class="close" data-dismiss="modal"><span>×</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        ...
                        {{ csrf_field() }}
                        <div class="form-group ">
                            <label class="col-form-label" for="passowrd">Old Passowrd <span class="text-danger">*</span>
                            </label>
                            <input type="password" class="form-control input-flat" id="old_password" name="old_password" placeholder="">
                            <div id="old_password-error" class="invalid-feedback animated fadeInDown" style="display: none;"></div>
                        </div>
                        <div class="form-group ">
                            <label class="col-form-label" for="passowrd">New Passowrd <span class="text-danger">*</span>
                            </label>
                            <input type="password" class="form-control input-flat" id="new_password" name="new_password" placeholder="">
                            <div id="new_password-error" class="invalid-feedback animated fadeInDown" style="display: none;"></div>
                        </div>
                        <div class="form-group ">
                            <label class="col-form-label" for="passowrd">Re-enter New Passowrd <span class="text-danger">*</span>
                            </label>
                            <input type="password" class="form-control input-flat" id="confirm_new_password" name="confirm_new_password" placeholder="">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-outline-primary" id="save_passwordBtn" data-id="{{ $user->id }}">Save <i class="fa fa-circle-o-notch fa-spin loadericonfa" style="display:none;"></i></button>
                    </div>
                </form>
            </div>
        </div>
...
$('#ChangePwdModal').on('hidden.bs.modal', function () {
    ...
    $('#confirm_new_password-error').html("");
});
...
$('body').on('click', '#EditProfileBtn', function () {
    ...
    $.get("{{ url('user') }}" +'/' + user_id +'/edit', function (data) {
        ...
        if(data.profile_pic==null){
            ...
            var profile_pic = "{{ env('DISPLAY_IMAGE_URL') }}" + "profile_pic/" + data.profile_pic;
...
$('body').on('click', '#ChangePwdBtn', function () {
    var user_id = $(this).attr('data-id');
    $.get("{{ url('user_password') }}" +'/' + user_id +'/edit', function (data) {
        $('#password_user_id').val(data.id);
        // $('#old_password').val(data.decrypted_password);
    })
});
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if(res.status == 200){
                ...
                $(btn).prop('disabled',false);
                $(btn).find('.loadericonfa').hide();
                $("#full_name").html(res.user.first_name+" "+res.user.last_name);
                ...
                $('#Profile_pic_val').attr('src', "{{env('DISPLAY_IMAGE_URL')}}" + 'profile_pic/' + res.user.profile_pic);
...
$('body').on('click', '#save_passwordBtn', function () {
    var btn = $(this);
    $(btn).prop('disabled',true);
    $(btn).find('.loadericonfa').show();
    $("#old_password-error").hide().text("");
    $("#new_password-error").hide().text("");
    $("#confirm_new_password-error").hide().text("");
        ...
        success: function (res) {
            if(res.status == 'failed'){
                ...
                if (res.errors.old_password) {
                    $('#old_password-error').show().text(res.errors.old_password);
                } else {
            ...
            if(res.status == 'failed_old_password'){
                $(btn).prop('disabled',false);
                $(btn).find('.loadericonfa').hide();
                if (res.error) {
                    $('#old_password-error').show().text(res.error);
                } else {
                    $('#old_password-error').hide();
                }
            }
